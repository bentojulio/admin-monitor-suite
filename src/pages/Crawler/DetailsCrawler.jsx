...
export default function DetailsCrawler() {
  ...
  const handleSelectAll = (field) => {
    ...
    const newData = data.map(item => {
      ...
      const isInPaginatedData = paginatedData.some(paginatedItem => paginatedItem.Uri === item.Uri);
  ...
  const handleSelectAllFiltered = (field) => {
    ...
    const newData = data.map(item => {
      const isInFilteredData = filteredData.some(filteredItem => filteredItem.Uri === item.Uri);
      if (isInFilteredData) {
        return {
          ...item,
    ...
    <div>
      ...
      <div className="content bg-white">
        ...
        <ul>
          ...
          <li>Nº de Páginas na amostra: {pageNumber}(nº de páginas que existem atualmente no sítio Web)</li>

        </ul>
        <p>O robô encontrou a lista de páginas abaixo. Selecione as páginas que pretende importar.</p>
        <p>Nota: o processo de importação só vai importar os URLs que ainda não existem na amostra.</p>
              ...
              {paginatedData.length > 0 ? (
                ...
                <tr>
                  <td colSpan="2" className="text-center py-4">
                    {searchTerm ? 'Nenhuma página encontrada para o termo pesquisado.' : 'Nenhuma página disponível.'}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="d-flex justify-content-between align-items-center mt-3 mb-3 pagination">
          <div>
            {totalItems > 0 && (
              <span className="ms-3">
                Mostrando {startIndex + 1} a {Math.min(endIndex, totalItems)} de {totalItems} páginas
              </span>
            )}
          </div>
